...
export function createAppsProvider(deps: AppsProviderDeps): ResourceProvider {
  ...
  return {
    ...
    async search(query: string): Promise<Result[]> {
      const q = query.trim()
      // A failed app list must not blank the whole All tab — the aggregator
      // guards each provider, but an empty Apps section is the better local
      // outcome than a rejected search.
      let apps: AppNavRecord[]
      try {
