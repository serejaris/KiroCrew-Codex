...
describe('createAppsProvider — failure handling', () => {
  it('yields an empty list when the app list cannot be fetched', async () => {
    // The All aggregator guards each provider, but returning [] keeps a failed
    // app list from being the thing that blanks the Apps tab with an error.
    const { d } = deps(async () => {
      throw new Error('gateway restarting')
    })
    expect(await createAppsProvider(d).search('fleet')).toEqual([])
  })
