...
/** Humanize a kebab/snake-case skill key for display. */
const displayName = (key: string) =>
  key.split('/').pop()!.replace(/[-_]/g, ' ').replace(/\b\w/g, c => c.toUpperCase())

/** Format chars as human-readable (e.g. 48.6M, 14K). */
...
export default function SkillContextBudget({ onBack }: { onBack: () => void }) {
  ...
  // Group rows
  const hotRows = rows.filter(r => r.inject_on_trigger && r.chars > 0).sort(comparators[sortBy])
  ...
  const maxChars = rows.length > 0 ? Math.max(...rows.map(r => r.chars)) : 1
  const hotSpend = hotRows.reduce((s, r) => s + r.chars, 0)
  const hotPct = totalChars > 0 ? Math.round(hotSpend / totalChars * 100) : 0

  // Top 3 percentage (for danger color)
  const top3Sorted = [...rows].sort((a, b) => b.chars - a.chars).slice(0, 3)
  const top3Chars = top3Sorted.reduce((s, r) => s + r.chars, 0)
  const top3Pct = totalChars > 0 ? Math.round(top3Chars / totalChars * 100) : 0

  const coldKB = fmtBytes(coldRows.reduce((s, r) => s + r.size_bytes, 0))
  const frozenChars = fmtChars(frozenRows.reduce((s, r) => s + r.chars, 0))
  ...
  if (rows.length === 0) {
    return (
      <div className="mt-4">
        ...
        <Card>
          <EmptyState
            icon={<ArrowLeft className="lucide-inline" />}
      ...
      <Card>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <div className="text-[12.5px] text-muted">
            <span className="text-accent font-bold">{fmtChars(totalChars)}</span>
            {' '}{i18nT('pages.overview.skillsTab.budget_header_chars')}{' '}
            <span className="text-danger font-semibold">{top3Pct}%</span>
          </div>
        ...
        <GroupHeader
          ...
          right={i18nT('pages.overview.skillsTab.budget_group_counting_detail', { count: String(hotRows.length), pct: String(hotPct) })}
        />
        {hotRows.map(row => (
          <BudgetRow
            key={row.key}
...
function BudgetRow({
  ...
  const barPct = maxChars > 0 ? Math.max(0.5, (row.chars / maxChars) * 100) : 0
        ...
        {cold ? (
          ...
          <>
            ...
            <div className="text-[10.5px] font-mono mt-1 text-muted">
              {fmtChars(row.chars)} {i18nT('pages.overview.skillsTab.budget_chars')}
            </div>
          </>
        )}
      </div>
