...
async def _github_stale_dismissal_enabled(ref: SourceRef, base_ref: str) -> bool:
    ...
    Fail CLOSED: an unreadable protection block (no admin rights, no protection at
    all, any error) returns False, which withholds APPROVE rather than assuming the
    branch is safe.
    """
    if not base_ref:
        return False
    try:
        ...
        logger.debug("branch protection unreadable for %s", ref.url, exc_info=True)
        return False
    if not isinstance(payload, dict):
        return False
