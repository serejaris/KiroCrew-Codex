...
def _restored_tab(state):
    """The slot as a restart rebuilds it: rehydrated from the channel transcript.

    Built through the real restore helper rather than ``get_or_create_slot`` so
    the slot carries the window a save actually persists — an empty slot returns
    early from ``_save_slot_to_history`` and would make these assertions vacuous.
    """
    slot = _rehydrate_slot_from_history(state, STEM)
    assert slot is not None
    assert slot.messages, "rehydrated slot must carry a window"
    return slot
...
class TestSlotHistoryKey:
    def test_unbound_channel_slot_resolves_to_its_channel_transcript(self, tmp_path):
        """The regression: this used to answer "dashboard:slack_<ts>"."""
        state = _state(tmp_path)
        slot = state.get_or_create_slot(STEM)
...
class TestChannelBindingIsValidated:
    ...
    def test_binds_a_real_channel_key(self, tmp_path):
        state = _state(tmp_path, resolves=CHANNEL_KEY)
        assert state.get_or_create_slot(STEM).linked_session_key == CHANNEL_KEY

    def test_leaves_it_unbound_when_the_map_cannot_resolve(self, tmp_path):
        state = _state(tmp_path)
        assert state.get_or_create_slot(STEM).linked_session_key == ""
